...
if (isset($_POST['systemsok']) ) {
	

	
	$sokeord = $_POST['sokSystem'];
	$sokeord = "%" . $sokeord . "%";
	$statement = $db->prepare("SELECT id, arkivskaper, status, startDato, 
 sluttDato, saksNR, mottattDATO,forventetAnkomst, system, ansvarlig, sistendret FROM uttrekk WHERE system LIKE ? ORDER BY startDato ASC");
	$statement->bind_param("s", $sokeord);
	$statement->execute();
    
	$resultat = $statement->get_result();
	$antall = $resultat->num_rows;
	
	echo "Antall treff: " . $antall . "<br /><br />";
